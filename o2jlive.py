...
for lineRead in configLines:
    if ( lineRead != "" ) and ( lineRead.strip()[0:1] != '#' ):
        ...
        if lineRead.split()[CONFIG_PROPERTY_ARG] == 'o2jlive.verbose_frame_num':
            ...
            verboseFrameNum = bool(
                    int(
                        lineRead.split()[CONFIG_VALUE_ARG]
                        )
                    )           
        ...
        if lineRead.split()[CONFIG_PROPERTY_ARG] == 'o2jlive.verbose_osc_listen_port':
            ...
            verboseOscListenPort = bool(
                    lineRead.split()[CONFIG_VALUE_ARG]
                    )

        if lineRead.split()[CONFIG_PROPERTY_ARG] == 'o2jlive.osc_server':
            oscServerId.append(
                    lineRead.split()[CONFIG_VALUE_ARG].split(':')
                    )
...
jackLatency = int(
        (jack_client.blocksize * 2 / jack_client.samplerate ) *1000
        )

if verboseLatency == True:
    print(
            'JACK latency detected at:', 
            end=' '
            )
    print(
            jackLatency, 
...
for oscServerNum in range(
        len(oscServerId)
        ):

    #make port number an int
    oscServerId[oscServerNum][OSC_CLIENT_PORT_INDEX] = int(
            oscServerId[oscServerNum][OSC_CLIENT_PORT_INDEX]
            )
    ...
    if verboseOscServers == True:
        ...
        print(
                oscServerId[oscServerNum][OSC_CLIENT_IP_INDEX] + 
                ":" + 
                str(
                    oscServerId[oscServerNum][OSC_CLIENT_PORT_INDEX]
                    )
...
if basicMode == False:
    try:
            ...
            if fileLineNum == 1:
                ...
                if lineRead!='' and lineRead.strip()[0:1] != '#':
                    ...
                    if lineRead.strip()[0:1] == '*':
                        ...
                        for listElement in range(
                                1,
                                len(timeEventList)
                                ):

                            timeEventList[listElement] = int(
                                    timeEventList[listElement]
                                    )

                        timeFrameNum = int(
                                timeEventList[FRAMENUM_INDEX][1:]
                                )
                        timeEventData = [
                                timeFrameNum ,
                                TIME_EVENT_TYPE ,
                    ...
                    if lineRead.strip()[0:1] == '$':
                        ...
                        loopStartMarker.append(
                                int(
                                    lineRead.split()[FRAMENUM_INDEX][1:]
                                    )
                                )
                    ...
                    if lineRead.strip()[0:1] == '!':
                        ...
                        loopJumpEventList[JUMP_TARGET_FILE_INDEX] = int(
                                loopJumpEventList[JUMP_TARGET_FILE_INDEX]
                                )
                        loopJumpFrameNum = int(
                                loopJumpEventList[FRAMENUM_INDEX][1:]
                                )
                        loopJumpEventData = [
                                loopJumpFrameNum , 
                                LOOP_JUMP_EVENT_TYPE ,
                                ]
                    ...
                    if lineRead.strip()[0:1] == '@':
                        ...
                        for listElement in range(
                            ...
                            oscEventList[listElement]=int(
                                    oscEventList[listElement]
                                    )
                        oscFrameNum = int(
                                oscEventList[FRAMENUM_INDEX][1:]
                                )
                        oscEventData = [
                                oscFrameNum , 
                                OSC_EVENT_TYPE ,
                    ...
                    if lineRead.strip()[0:1] == '/':
                        ...
                        for listElement in range(
                            ...
                            try:
                                oscMessage[listElement] = int(
                                        oscMessage[listElement]
                                        )
                            except:
                                try:
                                    oscMessage[listElement] = float(
                                            oscMessage[listElement]
                                            )
                                except:
                                    oscMessage[listElement] = str(
                                            oscMessage[listElement]
                                            )
        ...
        if verboseLoadedData == True:
            for loopStartNum in range(
                    len(loopStartMarker)
                    ):
                print(
                        'Loop Start point at frame' , 
                        end=' '
                        )
                print(loopStartMarker[loopStartNum])
        if timeSigSet == True:
            bpm = int(
                    eventData[initTimeData][EVENT_DATA_INDEX][TIME_BPM_INDEX]
                    )
            timeSigN = int(
                    eventData[initTimeData][EVENT_DATA_INDEX][TIME_SIGN_INDEX]
                    )
            timeSigD = int(
                    eventData[initTimeData][EVENT_DATA_INDEX][TIME_SIGD_INDEX]
                    )            
        else:
    ...
    for eventNum in range(
        ...
        if eventData[eventNum][FRAMENUM_INDEX] > trackFrame[PREVIOUS_FRAME_INDEX] and eventData[eventNum][FRAMENUM_INDEX] <= trackFrame[CURRENT_FRAME_INDEX] and eventData[eventNum][EVENT_TYPE_INDEX] != LOOP_JUMP_EVENT_TYPE:
            ...
            if eventData[eventNum][EVENT_TYPE_INDEX] == TIME_EVENT_TYPE:
                timeChange(
                        eventData[eventNum][EVENT_DATA_INDEX][TIME_BPM_INDEX] ,
                        eventData[eventNum][EVENT_DATA_INDEX][TIME_SIGN_INDEX] ,
                        eventData[eventNum][EVENT_DATA_INDEX][TIME_SIGD_INDEX] ,
                        )

                #verbose time change events
                if verboseEventData == True:
                    print(
                            'Time changed at frame ' + 
                            str(eventData[eventNum][FRAMENUM_INDEX]) + 
                            ' to ' + 
                            str(eventData[eventNum][EVENT_DATA_INDEX][TIME_SIGN_INDEX]) + 
                                    '/' + str(eventData[eventNum][EVENT_DATA_INDEX][TIME_SIGD_INDEX]) +
                                    ' @ ' + 
                                    str(eventData[eventNum][EVENT_DATA_INDEX][TIME_BPM_INDEX]) +
                                    'bpm'
                                    )
            #send osc events
            if eventData[eventNum][EVENT_TYPE_INDEX] == OSC_EVENT_TYPE:
                for message in range(
                        len(
                            eventData[eventNum][EVENT_DATA_INDEX][OSC_MESSAGES_INDEX]
                            )
                        ):
                    sendOSC(
                            eventData[eventNum][EVENT_DATA_INDEX][OSC_TARGET_INDEX] ,
                            eventData[eventNum][EVENT_DATA_INDEX][OSC_MESSAGES_INDEX][message][0] ,
                            eventData[eventNum][EVENT_DATA_INDEX][OSC_MESSAGES_INDEX][message][1:]
                            )
...
def moveTransport(location):
    """ use this function to both move the transport, and confirm that the transport has moved """
    ...
    #check that the transport is at the corect location
    ...
    if newTime != currentTime:
        timeChange(
                newTime[TIME_BPM_INDEX] ,
                newTime[TIME_SIGN_INDEX] ,
                newTime[TIME_SIGD_INDEX]
                )
...
def calcBarBeatToFrames(
        bar , 
        beat ,
        ):
    """ This function takes a bar and beat and calculates a frame number calculate beatLen and barLen. """
    ...
    return int(
            ( bar * barLenFrames ) + ( beat * beatLenFrames )
            )
...
def calcFramesToBarBeat(frame):
    ...
    #beat:
    beat = int(
            ( frame - ( bar * barLenFrames ) ) / beatLenFrames
            )
    barBeatNum = [
            bar , 
            beat ,
        ...
        if eventData[eventNum][FRAMENUM_INDEX] - jack_client.blocksize * transportLatencyCalibrate > trackFrame[PREVIOUS_FRAME_INDEX] and eventData[eventNum][FRAMENUM_INDEX] - jack_client.blocksize * transportLatencyCalibrate <= trackFrame[CURRENT_FRAME_INDEX] and eventData[eventNum][EVENT_TYPE_INDEX] == LOOP_JUMP_EVENT_TYPE and bypassLoops == False:
            if bypassLoopsOnce == False:
                if overrideJumpEvent == False:
                    ...
                    moveTransport(
                            loopStartMarker[
                                eventData[eventNum][EVENT_DATA_INDEX][JUMP_TARGET_INDEX]
                                ]
                            )

                    if verboseEventData == True:
                        print(
                                'Transport jumped to frame' , 
                                end=' '
                                )
                        print(
                                loopStartMarker[
                    ...
                    #verbose jump event
                    if verboseEventData == True:
                        print( 'Transport jumped to frame' , 
                                end=' '
                                )
...
def loopJumpOverride(
    ...
    if argNewJumpTarget != NULL_JUMP_TARGET and argNewJumpTarget <= len(loopStartMarker) - 1:
        ...
        if verboseOscIn == True:
            print(
                    'Loop Override!  Next loop starting point is' , 
                    end=' '
                    )
            print(newJumpTarget)
    return
...
def verboseTime():
    '''
    There are some known issues involving time changes
    ...
    if verboseBarBeat == True:
        if calcFramesToBarBeat(
                trackFrame[PREVIOUS_FRAME_INDEX]
                ) != calcFramesToBarBeat(
                        trackFrame[CURRENT_FRAME_INDEX]
                        ):
            print(
                    calcFramesToBarBeat(
                        trackFrame[CURRENT_FRAME_INDEX]
...
def jackTransport(
    ...
    if verboseOscIn == True:
        print(
                ...
                " " +
                str(
                    args[JACK_TRANSPORT_STATE_OSC_ARG_INDEX]
                    )
                )

    #capture args
    jackTransportState = int(
            args[JACK_TRANSPORT_STATE_OSC_ARG_INDEX]
            )
...
def jackRemoteTransport(
    ...
    if verboseOscIn == True:
        print(
                ...
                " " +
                str(
                    args[JACK_REMOTE_TRANSPORT_STATE_OSC_ARG_INDEX]
                    )
                )
    #capture args
    jackTransportState = int(
    ...
    serverTargetId = int(
            args[JACK_REMOTE_TRANSPORT_SERVER_ID_OSC_ARG_INDEX]
            )
...
def jackAllTransport(
    ...
    if verboseOscIn == True:
        print(
                ...
                ' ' +
                str(
                    args[JACK_ALL_TRANSPORT_STATE_OSC_ARG_INDEX]
                    )
                )
...
def loopJumpBypass(
    ...
    if verboseOscIn == True:
        print(
                ...
                " " +
                str(
                    args[LOOP_JUMP_BYPASS_OSC_ARG_INDEX]
                    )
                )
...
def loopJumpBypassOnce(
    ...
    if verboseOscIn == True:
        print(
                ...
                " " +
                str(
                    args[LOOP_JUMP_BYPASS_ONCE_OSC_ARG_INDEX]
                    )
                )

    global bypassLoopsOnce
    bypassLoopsOnce = bool(
            args[LOOP_JUMP_BYPASS_ONCE_OSC_ARG_INDEX]
            )
    return
...
osc_server.add_method(
        JACK_ALL_TRANSPORT_PATH , 
        None ,
        jackAllTransport
        )

osc_server.add_method(
        JACK_REMOTE_TRANSPORT_PATH ,
        None ,
        jackRemoteTransport
        )
...
osc_server.add_method(
        LOOP_JUMP_OVERRIDE_PATH ,
        None ,
        loopJumpOverride
        )

osc_server.add_method(
        LOOP_JUMP_BYPASS_PATH ,
        None ,
        loopJumpBypass
        )

osc_server.add_method(
        LOOP_JUMP_BYPASS_ONCE_PATH ,
        None ,
        loopJumpBypassOnce
        )
...
if __name__ == '__main__':
    ...
    while mainLoop:
        ...
        if trackFrame[CURRENT_FRAME_INDEX] != lastOtherEventsFrame and not(
                transportMoved
                ):
            checkOtherEvents()
        else:
